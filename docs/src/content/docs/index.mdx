---
title: 'Sygnal'
description: 'An intuitive reactive component framework built on Cycle.js patterns.'
template: splash
hero:
  tagline: An intuitive reactive component framework built on Cycle.js patterns. Pure components, automatic state management, and zero boilerplate.
  actions:
    - text: Get Started
      link: /getting-started/
      icon: right-arrow
      variant: primary
    - text: View on GitHub
      link: https://github.com/tpresley/sygnal
      icon: external
      variant: minimal
---

import { Card, CardGrid } from '@astrojs/starlight/components';

## Why Sygnal?

<CardGrid stagger>
  <Card title="MVI Architecture" icon="pencil">
    Model-View-Intent separates *what* happens, *when* it happens, and *how* it looks. Clear data flow makes components easy to reason about and debug.
  </Card>

  <Card title="Reactive Streams" icon="random">
    Built on xstream and Cycle.js patterns. Every interaction is a stream, giving you powerful composition and transformation with minimal code.
  </Card>

  <Card title="Zero Boilerplate" icon="rocket">
    No hooks, no selectors, no dispatchers. Define `initialState`, `intent`, and `model` as static properties and Sygnal wires everything together.
  </Card>

  <Card title="Pure Components" icon="approve-check">
    View functions are pure — they receive state, return JSX. No side effects, no lifecycle methods, no `this`. Simple to test and simple to compose.
  </Card>

  <Card title="Collections" icon="list-format">
    Render dynamic lists of components with automatic isolation, keyed reconciliation, and built-in parent/child communication.
  </Card>

  <Card title="Astro Integration" icon="astro">
    First-class Astro support with server-side rendering and client hydration. Drop Sygnal components into any Astro page with a single directive.
  </Card>

  <Card title="Vike Integration" icon="seti:html">
    File-based routing with SSR, client-side navigation, and automatic hydration via [Vike](https://vike.dev). Layouts, data fetching, and SPA mode included.
  </Card>
</CardGrid>
